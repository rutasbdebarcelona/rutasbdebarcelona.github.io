...
function setup(root:HTMLElement){
  ...
  form.addEventListener('submit',event=>{event.preventDefault();void save(false)});$('[data-pages-publish]')?.addEventListener('click',()=>{if(confirm('Â¿Publicar los contenidos de todas las pÃ¡ginas en castellano e inglÃ©s?'))void save(true)});
  void getAdminSiteDesign().then(data=>fill(data.draft||data.published,data.draft?'Borrador pendiente':'VersiÃ³n pÃºblica vigente')).catch((error:any)=>{const message=$('[data-pages-message]');if(message)message.textContent=error.message||'No fue posible cargar los contenidos.';});
}
