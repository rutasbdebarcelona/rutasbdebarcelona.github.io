...
export function setupRouteProductEditor(root){
 ...
 const load=data=>{
  ...
  set('productScheduleEn',en.schedule_notes);set('meetingReferenceEn',en.meeting_reference);set('meetingInstructionsEn',en.meeting_instructions);set('meetingTransportEn',en.meeting_transport);
 ...
 const serialize=()=>{
  ...
  const product={
   ...
   translations:{en:{meeting_reference:form.elements.meetingReferenceEn.value.trim(),short_description:form.elements.productShortDescriptionEn.value.trim(),long_description:form.elements.productLongDescriptionEn.value.trim(),highlights:lines(form.elements.productHighlightsEn.value),itinerary_summary:form.elements.productItineraryEn.value.trim(),schedule_notes:form.elements.productScheduleEn.value.trim(),meeting_instructions:form.elements.meetingInstructionsEn.value.trim(),meeting_transport:form.elements.meetingTransportEn.value.trim()}}
  };
